 ...
 * real children via authHelperPath. No paid auth or live workers.
 *
...
test("home redraw and failed apply keep the captured target", async () => {
  const world = await makeWorld("cma-ms-target-");
  await writeConfig(world.configFile, apiConfig());
  const replacementRoot = path.join(world.root, "replacement-worker");
  const original = {
    ok: true,
    protocolVersion: 2,
    sessionId: world.sessionId,
    projectRoot: world.root,
    configPath: world.configFile,
    workerGeneration: 1,
    settingsRevision: 4,
    enabled: false,
    paused: false,
    advisors: [{ name: "correctness", enabled: true, model: "gpt-old", reasoningEffort: "default" }]
  };
  const replacement = {
    ...original,
    ...
    settingsRevision: 50,
    enabled: true,
    advisors: [{ name: "correctness", enabled: true, model: "gpt-replaced", reasoningEffort: "default" }]
  };
  let settingsCalls = 0;
  const applies = [];
  const homes = [];
  ...
  const ui = scriptedUi((kind, opts) => {
    if (kind === "choose" && /^Advisor /.test(String(opts.title))) return null;
    if (kind === "choose" && isHome(opts)) {
      homes.push({
        title: String(opts.title),
        values: (opts.items ?? []).map((item) => item.value),
        descriptions: (opts.items ?? []).map((item) => String(item.description ?? ""))
      });
      if (!toggled) {
        toggled = true;
        return { action: "toggle", value: "correctness" };
      }
      if (!openedEditor) {
        openedEditor = true;
      ...
      if (homes.length > 8) return select(opts.items, "Quit");
      const values = homes.at(-1).values;
      if (values.includes("Save & Apply")) return select(opts.items, "Save & Apply");
      if (values.includes("Enable")) return select(opts.items, "Enable");
      return select(opts.items, "Quit");
    }
    throw new Error(`unexpected ${kind}\n${dumpItems(opts)}`);
  });
  const code = await runMenu(world, ui, {
    getSessionSettings: async () => {
      settingsCalls += 1;
      return settingsCalls === 1 ? original : replacement;
    },
    applySessionSettings: async (options) => {
      applies.push({
        workerGeneration: options.target?.workerGeneration,
        settingsRevision: options.target?.settingsRevision,
        projectRoot: options.target?.projectRoot,
        enable: options.enable === true
      });
      return { ok: false, error: "stale" };
    },
    saveConfig: async () => ({ ok: true, path: world.configFile, revision: "rev-after-save" })
  });
  assert.equal(code, 0);
  ...
  const joinedTitles = homes.map((home) => home.title).join("\n");
  const joinedDescriptions = homes.flatMap((home) => home.descriptions).join("\n");
  assert.equal(joinedTitles.includes("replacement-worker"), false);
  assert.equal(joinedTitles.includes("Running"), false);
  assert.equal(joinedDescriptions.includes("gpt-replaced"), false);
  const afterFail = homes.slice(3);
  assert.ok(afterFail.length >= 1);
  for (const home of afterFail) {
    assert.equal(home.values.includes("Save & Apply"), false);
    assert.equal(home.values.includes("Enable"), false);
  }
...
test("enable of empty or all-disabled result does not report enabled", async () => {
  const cases = [
    {
      prefix: "cma-ms-empty-",
      config: apiConfig({ advisors: [] }),
      advisors: []
    },
    {
      prefix: "cma-ms-off-",
      ...
      advisors: [{ name: "correctness", enabled: false, model: "gpt-old", reasoningEffort: "default" }]
    }
  ];
  for (const fixture of cases) {
    const world = await makeWorld(fixture.prefix);
    await writeConfig(world.configFile, fixture.config);
    const applies = [];
    const ui = scriptedUi((kind, opts) => {
      if (kind === "choose" && isHome(opts)) {
        if (!ui._enable) {
          ui._enable = true;
          return select(opts.items, "Enable");
        }
        return select(opts.items, "Quit");
      }
      throw new Error(`unexpected ${kind}\n${dumpItems(opts)}`);
    ...
    const captured = {
      ...
      enabled: false,
      paused: false,
      advisors: fixture.advisors
    };
    const code = await runMenu(world, ui, {
      getSessionSettings: async () => captured,
      applySessionSettings: async (options) => {
        applies.push({
          enable: options.enable === true,
          workerGeneration: options.target?.workerGeneration,
        ...
        return {
          ...
          ok: true,
          enabled: false,
          settingsRevision: captured.settingsRevision + 1,
          advisors: fixture.advisors
        };
      }
    });
    assert.equal(code, 0);
    assert.deepEqual(applies, [{ enable: true, workerGeneration: 1, settingsRevision: 3 }]);
    const enableNotices = ui.notices.filter((note) => note.title === "Enable");
    assert.ok(enableNotices.some((note) => note.text === "Session remains off."));
    assert.equal(
      enableNotices.some((note) => /enabled this session/i.test(String(note.text))),
      false
    );
  }
});
