...
2. **Show exactly what will be removed** — every worktree path and branch name, and which repository they live in, per PR. A merged evidence context has both its contributor-head worktree and an additional `-landed` worktree at GitHub's merge commit. This matters because removal touches the user's real repositories, not just the cache: worktree registrations and `codex-pr/*` branches are created inside whichever repo hosted the PR.
   ...
   Every result also carries `running` — primary or context entries a Codex review is still reading. All participating entries are left alone (`held: true`) rather than deleted from under the run, including a merged context's landed snapshot. Name them, say the cleanup can be repeated once they finish, and do not offer `--include-running` unless the user asks for it: it lets the removal go ahead, which can make the running review fail and waste a paid run.
...
- A PR whose checkout is still part of a running review is skipped, worktrees and branches included: the run is reading that evidence, and its output lands after any plan made here. `--include-running` overrides that. The run then re-records its primary and context entries when it saves, so the review stays reachable — but it may well have lost a checkout it was reading, so the override is for a marker left behind by a crashed run, not for hurrying a live one along.
