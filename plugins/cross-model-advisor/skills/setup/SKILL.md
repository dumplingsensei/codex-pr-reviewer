...
2. Ask: **Add an advisor** · **Change an advisor** · **Gate settings** · **Terminal menu**.
   ...
   - **Gate settings**: **Mode** (block/report/advise/watch) · **Max rounds** (1–5) · **Auto-on projects** · **Skip patterns**. Auto-on: offer adding or removing this session's absolute git root; other absolute paths via Other. Skip: `*.md`, `docs/**`, or Other, never starting with `!`. The change is `{"op":"set-gate","gate":{…}}`; lists replace whole, and an empty list removes the setting.
