...
- `lastReview`: when it ran, its `outcome` (`blocked` sent Claude back, `woke` woke Claude from a background review, `steered` interrupted Claude mid-turn, `reported` showed findings to the user, `passed` found nothing, `failed` could not review), the review round, every finding with its advisor, severity, note, and evidence, and each advisor's result or error
