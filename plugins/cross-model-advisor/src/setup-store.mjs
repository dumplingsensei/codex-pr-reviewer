/**
 * Offline setup service: catalog, bounded model lists, revision-checked
 * private config persistence, and shared read/save helpers for the terminal
 * menu. No login, no network, no credential reads, no session activation.
 */
...
const COMPATIBLE_HINT =
  "openai-compatible has no offline SDK catalog. Reuse model metadata from the current config or collect contextWindow, maxTokens, reasoning, and input from the user. Optional paired thinkingFormat (openai|openrouter|zai) and thinkingLevelMap covering off/minimal/low/medium/high/xhigh/max; reasoning true alone is not tunable.";
...
export function parseSetupArgv(argv) {
  ...
  if (command === "efforts") {
    const [, providerId, kind, model] = argv;
    if (argv.length !== 4 || !isSupportedProviderId(providerId ?? "") || (kind !== "api" && kind !== "oauth")) {
      fail("usage", USAGE);
    }
    if (typeof model !== "string" || model.length === 0 || model.length > MAX_QUERY_CHARS) fail("usage", USAGE);
    return { command, providerId, kind, model };
  }
...
/**
 * @param {string} file
 * @param {(current: Awaited<ReturnType<typeof readConfigStateFromPath>>) => Promise<unknown>} fn
 */
 ...
 */
export async function readConfigState({ env = process.env } = {}) {
...
/**
 * Offline provider summaries for the terminal menu. Same catalog as `runSetup`
 * `{argv:['catalog']}` without subprocess parsing.
 *
 * @param {{ createBuiltinProvider?: (id: string) => object | Promise<object> }} [options]
 */
 ...
 *   offset?: number,
 *   limit?: number,
 *   createBuiltinProvider?: (id: string) => object | Promise<object>
 * }} [options]
 */
export async function getModels(providerId, options = {}) {
 ...
 * @param {(providerSlot: object, advisor: object, maxOutputTokens: number) => Promise<{ ok: true } | { ok: false, error?: string }>} validateReasoningFn
 */
...
/**
 * @param {{ revision: string | null, config: unknown }} payload
 * @param {{
 *   env?: NodeJS.ProcessEnv,
 *   validateReasoning?: typeof defaultValidateReasoning
 * }} [options]
 * @returns {Promise<{ ok: true, path: string, revision: string }>}
 */
export async function saveConfig(payload, options = {}) {
...
/**
 * Role instructions offered by the chat setup (`/cross-model-advisor:setup`).
 * Literal text, as the menu's instruction editor would store it.
 */
...
const APPLY_KEYS = Object.freeze(["revision", "change"]);
const ADVISOR_SET_KEYS = Object.freeze(["model", "reasoningEffort", "instructions", "enabled"]);
const GATE_SET_KEYS = Object.freeze(["mode", "maxRounds", "autoOn", "skipWhenOnly"]);
const TERMINAL_ONLY =
  "Custom OpenAI-compatible endpoints are set up in the terminal menu, where the URL and model metadata are entered.";

/** Revision prefix the chat setup shows and sends: 64 bits of the file's hash. */
const SHORT_REVISION = 16;

/**
 * What the chat setup shows: advisors, slots, and gate, compactly. Key
 * variable names only, never values. Providers come from `providers`, preset
 * texts stay in the helper (`instructionsPreset`), and instructions are cut
 * here because `apply --dry-run` shows the full text before any save.
 *
 * @param {Awaited<ReturnType<typeof readConfigStateFromPath>>} state
 */
function summarizeSetup(state) {
  ...
  return {
    ...
    slots: config
      ? Object.entries(config.providers).map(([slot, entry]) => ({
          slot,
...
/**
 * `instructionsPreset: "<role>"` in place of `instructions`, so the chat setup
 * never has to copy a preset's text.
 *
...
/**
 * One structured change to a config, returning the changed copy. Nothing here
 * writes; `applyChange` validates the result and saves it.
 *
...
function withChange(config, change) {
  const next = config
    ...
    : { version: 2, providers: {}, advisors: [], exclude: [], limits: {} };
  next.version = 2;
...
/**
 * Human-readable lines for what a change does, shown before it is saved.
 *
 * @param {any} before
 * @param {any} after
 */
...
/**
 * Applies one change: validates the result as the gate would load it, checks
 * the model against the provider's offline catalog, and with `dryRun` stops
 * there. Otherwise saves through `saveConfig`, so the lock, revision check,
 * and atomic write are the menu's.
 *
 * @param {{ revision: string | null, change: Record<string, any> }} payload
 * @param {{ env?: NodeJS.ProcessEnv, dryRun?: boolean, createBuiltinProvider?: typeof defaultCreateBuiltinProvider,
 *   validateReasoning?: typeof defaultValidateReasoning }} [options]
 */
export async function applyChange(payload, options = {}) {
  const env = options.env ?? process.env;
  ...
  // The short revision summary prints (or the full one) must still prefix the file's.
  const matches =
    payload.revision === state.revision ||
    (typeof payload.revision === "string" &&
      typeof state.revision === "string" &&
      payload.revision.length >= SHORT_REVISION &&
      state.revision.startsWith(payload.revision));
...
export async function runSetup(options = {}) {
  ...
  if (parsed.command === "providers") {
    ...
    writeJson(stdout, {
      ok: true,
      providers: providers
        .filter((entry) => entry.id !== "openai-compatible")
        .map((entry) => ({ id: entry.id, name: entry.name, auth: entry.auth, ...(entry.suggestedApiKeyEnv ? { env: entry.suggestedApiKeyEnv } : {}) }))
    });
    return 0;
  }
  ...
  if (parsed.command === "efforts") {
    ...
    // Values only; an alias (Minimal sent as Low) is named once, not per choice.
  ...
  if (parsed.command === "apply") {
    ...
    writeJson(
      stdout,
      await applyChange(parseApplyPayload(raw), {
        env,
