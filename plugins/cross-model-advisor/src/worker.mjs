#!/usr/bin/env node
/**
 * Per-session worker. Owns serialized event ingestion, observation order,
 * bounded scheduling, cancellation, and finding delivery. Backends never
 * publish; hooks never signal stored PIDs.
 */
...
const PAUSE_IMMEDIATE = new Set(["rate", "auth", "config", "unsupported-tools", "context-limit"]);
...
/**
 * Hook cwd must canonicalize into the stored frozen root. The root path is
 * not recanonicalized: a retargeted root identity pauses.
 *
 * @param {string} cwd
 * @param {string} root
 * @param {{ path?: string, dev?: number, ino?: number } | null} [rootIdent]
 */
export function cwdInsideRoot(cwd, root, rootIdent) {
  ...
  try {
    ...
    if (
      ...
      rootIdent.dev != null &&
      rootIdent.ino != null &&
      (Number(listing.dev) !== Number(rootIdent.dev) || Number(listing.ino) !== Number(rootIdent.ino))
    ) {
      return false;
    }
...
const DIRECT_PROVIDER_KINDS = new Set(["api", "oauth"]);
const UNSUPPORTED_PROVIDER_MESSAGE =
  "saved CLI provider is no longer supported; run /cross-model-advisor:on to reconfigure";
    ...
    try {
      ...
      const locatorLive = Boolean(
        locator?.pid &&
          ...
          locator.socketPath &&
          fsSync.existsSync(locator.socketPath)
      );
...
export async function startWorker(options) {
  try {
    ...
    // umask may be restricted in some test sandboxes
  }
  ...
  const ingestTranscript = async (transcriptPath) => {
    ...
    for (const record of increment.records) {
      ...
      const uuidKey = interpreted.uuid
        ? dedupeKey({ sessionId, generation: state.generation, uuid: interpreted.uuid })
        : null;
    ...
    if (!state.latestTask) {
      const recovered =
        newestUser ??
        recoverLatestTask(increment.records, {
          secrets: secrets(),
          controlPromptIds
        });
      if (recovered?.text) {
  ...
  const publicationAllowed = (reserved) => {
    ...
    if (
      ...
      reserved.promptId !== state.latestTask.promptId
    ) {
      return false;
    }
    return true;
  ...
  const launchDue = async () => {
    if (
      ...
      state.paused ||
      state.primaryIdle ||
      state.contextUnavailable ||
      state.cwdOutsideRoot
    ) {
      return;
    }
  ...
  const handleSessionStart = async (payload) => {
    ...
    if (source === "compact") {
      ...
      const recovered =
        typeof payload.compact_summary === "string"
          ? payload.compact_summary
          : state.transcriptPath
            ? recoverCompactSummary((await readTranscriptIncrement(state.transcriptPath, { offset: 0 })).records, secrets())
            : null;
      if (recovered) {
  ...
  try {
    if (options.listen !== false) {
      server = await listenIpc(socket.socketPath, (req, respond) =>
        runSerial(async () => {
          try {
